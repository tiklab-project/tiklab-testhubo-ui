...
const WebSceneStepDrawer = ({name,stepId,findList}) =>{
    const {updateWebSceneStep,findWebSceneStep} = webSceneStepStore;
    ...
    const showDrawer = async () => {
        ...
        let info = await findWebSceneStep(stepId)
    ...
    const save = async () =>{
        ...
        updateWebSceneStep(values).then((res)=>{
            if(res.code===0){
                findList();
...
export default observer(WebSceneStepDrawer);