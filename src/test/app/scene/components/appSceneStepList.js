...
const AppSceneStepList = (props) => {
    ...
    useEffect(async ()=>{
        ...
        let actionTypeRes= await Axios.post("/actionType/findActionTypeList",{"type": "APP"});
    ...
    const deleteStep =async (id) =>{
        deleteAppSceneStep(id).then(async () => {
    ...
    const onFinish = async () =>{
        ...
        if(stepSelect){
            ...
            let res =await updateAppSceneStep(param)
            if(res.code===0){
                await findPage()
                setStepSelect(param)
            }
            ...
            let res = await createAppSceneStep(values)
            if(res.code===0){
                await findPage()
                values.id=res.data
