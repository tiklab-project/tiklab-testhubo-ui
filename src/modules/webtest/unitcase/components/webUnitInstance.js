...
const WebUnitInstance = (props) =>{
    const {webUnitInstanceStore} = props;
    const {findWebUnitInstanceList,instanceList,findWebUnitInstance} = webUnitInstanceStore;
    ...
    useEffect(()=>{
        findWebUnitInstanceList(1)
    },[])

    const clickFindInstance = id =>{
        setSelected(id)
        findWebUnitInstance(id).then(res=>{
...
export default inject("webUnitInstanceStore")(observer(WebUnitInstance));