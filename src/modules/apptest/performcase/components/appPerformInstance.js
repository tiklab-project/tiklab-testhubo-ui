...
const AppPerfInstance =(props)=>{
    const {appPerfInstanceStore} = props;
    const {findAppPerfInstanceList,findAppPerfInstance,appPerfInstanceList} = appPerfInstanceStore;
    ...
    useEffect(()=>{
        findAppPerfInstanceList(appPerfId)
    },[appPerfId])

    const clickFindInstance = id =>{
        setSelected(id)
        findAppPerfInstance(id).then(res=>{
            setResult(res)
        })
        setSceneDetail(null)
    }
    ...
    const goback = () =>{
        props.history.push("/repository/testcase/app-perform-detail")
    }
        ...
        <>
            ...
            <div className={"scene-instance-contant"}>
                <div className={"test-detail-history"}>
                    ...
                    {
                        showInstanceListView(appPerfInstanceList)
                    }
                </div>
...
export default inject("appPerfInstanceStore")(observer(AppPerfInstance));