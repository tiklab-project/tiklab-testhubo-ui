...
const AppPerformCofig = (props) =>{
    const {appPerformStore} = props;
    const {findAppPerform,appPerformInfo} = appPerformStore;
...

            {
                node==="mult"
                    ? <div className={`perfom-config-item perform-client-box`}>
                        ...
                        {
                            showClient(appPerformInfo?.nodeList)
                        }
                    </div>
                    :null
            }
...
export default inject("appPerformStore")(observer(AppPerformCofig));